...
namespace TestePrático
{
	public class Login
	{
		...
		public bool CadastrarTurma(int idProfessor, string nomeTurma)
		{
			if (TurmaExists(nomeTurma))
			{
				Console.WriteLine("Turma com o mesmo nome já existe.");
				return false;
			}
			...
			using (var connection = new MySqlConnection(conn))
			{
				...
				{
					connection.Open();
					using (var command = new MySqlCommand("INSERT INTO Turma (ProfessorId, Nome) VALUES (@IdProfessor, @NomeTurma)", connection))
					{
		...
		{
			if (AtividadeExists(turmaId, descricao))
			{
				Console.WriteLine("Atividade com o mesmo nome já existe na turma.");
				return false;
			}
		...
		private bool TurmaExists(string nomeTurma)
		{
			using (var connection = new MySqlConnection(conn))
			{
				connection.Open();
				using (var command = new MySqlCommand("SELECT COUNT(*) FROM Turma WHERE Nome = @NomeTurma", connection))
				{
					command.Parameters.AddWithValue("@NomeTurma", nomeTurma);
					return Convert.ToInt32(command.ExecuteScalar()) > 0;
				}
		...
		private bool AtividadeExists(int turmaId, string descricao)
		{
			using (var connection = new MySqlConnection(conn))
			{
				connection.Open();
				using (var command = new MySqlCommand("SELECT COUNT(*) FROM Atividade WHERE TurmaId = @TurmaId AND Descricao = @Descricao", connection))
				{
					command.Parameters.AddWithValue("@TurmaId", turmaId);
					command.Parameters.AddWithValue("@Descricao", descricao);
					return Convert.ToInt32(command.ExecuteScalar()) > 0;
